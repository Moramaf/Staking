...
//     }
//         //admin functions:
//     function changeRewardTime(uint _time) public onlyRole(ADMIN) { //onlyAdmin
//         rewardTime = _time;
//     }
//     function changeLockUpTime(uint _time) public onlyRole(ADMIN) { //onlyAdmin
//         lockUpTime = _time;
//     }
//      function changeRewardRate(uint _newRate) public onlyRole(ADMIN) { //onlyAdmin
//         rewardRate = _newRate;
//     }
// }
...
struct Balance {
    ...
    function stake(uint _amount) external updateBalance(msg.sender) updateReward(msg.sender) { //stake lp tokens 
        ...
        balances[msg.sender].timeToReward = block.timestamp + rewardTime * 1 minutes;
        totalSupply += _amount;
        stakingToken.transferFrom(msg.sender, address(this), _amount); // необходимао approve в контракте токена
    }

    function claim() external updateBalance(msg.sender) updateReward(msg.sender) { //withdraw reward
        uint reward = balances[msg.sender].claimableReward;
        require(reward>= 0, "Reward is not available");
        stakingToken.transfer(msg.sender, reward);
    ...
    function unstake(uint _amount) external updateBalance(msg.sender) updateReward(msg.sender) {
        require((balances[msg.sender].unclaimableBalance + balances[msg.sender].claimableBalance) > 0, "No tokens staked");
        uint unstakeAmount = balances[msg.sender].claimableBalance;
        require(unstakeAmount > 0, "Lockup time doesn't over");
        require(unstakeAmount > _amount, "Don't have enough tokens unstake");
        unstakeAmount -= _amount;
    ...
    modifier updateReward(address _account) { //update information about accessible reward in current time
        if((balances[_account].unclaimableBalance + balances[_account].claimableBalance) == 0) {
            _;
        } else {
            if(block.timestamp >= balances[_account].timeToReward) {
                balances[_account].claimableReward += balances[_account].pendingRewards;
